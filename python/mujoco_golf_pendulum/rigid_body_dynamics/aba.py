...
def aba(
    ...
    assert len(tau) == nb, f"tau must have length {nb}, got {len(tau)}"
    ...
    ia_articulated = [np.zeros((6, 6)) for _ in range(nb)]  # Articulated-body inertias
    ...
    for i in range(nb):
        ...
        if model["parent"][i] == -1:
            ...
            c[:, i] = crm(v[:, i]) @ vj_velocity  # Velocity-product acceleration
        ...
        pa_bias[:, i] = crf(v[:, i]) @ (model["I"][i] @ v[:, i]) - f_ext[:, i]
    ...
    for i in range(nb - 1, -1, -1):
        ...
        if model["parent"][i] != -1:
            ...
            if abs(d[i]) < 1e-10:
                d[i] = np.sign(d[i]) * 1e-10 if d[i] != 0 else 1e-10
