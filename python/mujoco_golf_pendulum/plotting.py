...
if TYPE_CHECKING:
    from .biomechanics import SwingRecorder
...
try:
    ...
    # Define a placeholder class that matches the Qt version signature
    # This ensures code that imports MplCanvas doesn't break even if Qt is unavailable
...
class GolfSwingPlotter:
    ...
    def get_joint_name(self, joint_idx: int) -> str:
        ...
        if joint_idx < 0 or joint_idx >= self.model.nq:
            return f"Joint {joint_idx}"

        name = mujoco.mj_id2name(self.model, mujoco.mjtObj.mjOBJ_JOINT, joint_idx)
        return name if name else f"Joint {joint_idx}"
    ...
    def plot_phase_diagram(self, fig: Figure, joint_idx: int = 0) -> None:
        ...
        if len(times) == 0 or positions.ndim < 2 or joint_idx >= positions.shape[1]:
            ax = fig.add_subplot(111)
