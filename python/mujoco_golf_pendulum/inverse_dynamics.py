...
class InverseDynamicsSolver:
    ...
    def compute_required_torques(
        ...
        # Compute Coriolis forces C(q,q̇)q̇
        ...
        # For parallel mechanisms, may need constraint force resolution
        constraint_forces = None
        if self.has_constraints:
            # Extract constraint forces
            constraint_forces = self.data.qfrc_constraint.copy()

        # Decompose coriolis into centrifugal and coupling
        _centrifugal, _ = self.kinematic_analyzer.decompose_coriolis_forces(qpos, qvel)
...
def export_inverse_dynamics_to_csv(
    ...
    with open(filepath, "w", newline="") as f:
        ...
        # Data rows
        for t, result in zip(times, results):  # noqa: B905
            row = [t]
